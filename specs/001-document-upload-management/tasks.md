...
**Source**: Post-implementation cleanup of document-upload-management feature  
**Priority**: Address before next feature iteration or when performance issues arise

### Detected Issues

- [X] TD001 Optimize `DocumentService.SearchDocumentsAsync` query pattern - Currently loads `sharedDocIds.ToList()` into memory before applying to the main query (line ~625); refactor to use a subquery for better performance with large share counts in `ContosoDashboard/Services/DocumentService.cs`
- [X] TD002 Refactor `DocumentService.ShareDocumentWithProjectAsync` to batch operations - Currently loops through project members calling `ShareDocumentAsync` individually (N+1 pattern at line ~517); optimize by batch-inserting `DocumentShare` records and sending notifications in bulk in `ContosoDashboard/Services/DocumentService.cs`
